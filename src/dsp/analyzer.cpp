...
// Rabiner, Gold: "The Theory and Application of Digital Signal Processing",
// 1975, page 103.
...
PlanckTaperWindow::PlanckTaperWindow(int size, int taperSamples)
  ...
  for (int i = 1; i < taperSamples; ++i) {
    float x = ((float)taperSamples / (float)i) -
              ((float)taperSamples / (float)(taperSamples - i));
    x = 1.0f + exp(x);
    x = 1.0f / x;
    _sum += _window[i] = x;
  }
...
FFT1024::FFT1024() { _fft = new FIXED_FFT1024(); }

FFT1024::~FFT1024() { delete (FIXED_FFT1024 *)_fft; }
...
FFT4096::FFT4096() { _fft = new FIXED_FFT4096(); }

FFT4096::~FFT4096() { delete (FIXED_FFT4096 *)_fft; }
...
FFT8192::~FFT8192() { delete (FIXED_FFT8192 *)_fft; }
...
FFT16384::FFT16384() { _fft = new FIXED_FFT16384(); }

FFT16384::~FFT16384() { delete (FIXED_FFT16384 *)_fft; }
...
FFT32768::FFT32768() { _fft = new FIXED_FFT32768(); }

FFT32768::~FFT32768() { delete (FIXED_FFT32768 *)_fft; }

void FFT32768::do_fft(float *out, float *in) {
  ((FIXED_FFT32768 *)_fft)->do_fft(out, in);
}

SpectrumAnalyzer::SpectrumAnalyzer(Size size, Overlap overlap,
                                   WindowType windowType, float sampleRate,
                                   bool autoProcess)
...
void SpectrumAnalyzer::getMagnitudes(float *bins, int nBins) {
  ...
  for (int bin = 0; bin < nBins; ++bin) {
    ...
    for (int i = binEnd - binWidth; i < binEnd; ++i) {
      sum +=
          (_fftOut[i] * _fftOut[i] + _fftOut[i + bands] * _fftOut[i + bands]) *
          normalization;
    }
