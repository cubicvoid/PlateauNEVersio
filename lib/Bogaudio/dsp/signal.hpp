...
struct RunningAverage {
	float _maxDelayMS = 300.f;
	float _sampleRate = -1.0f;
	float _sensitivity = -1.0f;

	bool _initialized = false;
	// float* _buffer = NULL;
	static const int bufferSize = 32000;
	...
	RunningAverage(float sampleRate = 32000.0f, float sensitivity = 1.0f, float maxDelayMS = 300.0f) : _maxDelayMS(maxDelayMS) {
